...
public class CarBL implements CarBLservice {
	...
	public ArrayList<CarVO> findAll() {
    	...
    	for(int i = 0; i<poList.size();i++)
    	{
    		...
    		CarVO vo=new CarVO(po.getVehicle(),po.getName(),po.getEngine(),po.getCarNum(),po.getBasenumber(),po.getBuytime(),po.getUsetime());
    		voList.add(vo);
    	}
		return voList;
	...
	public CarVO addCar(Vehicle vehicle, String name, long engine, String carNum, long basenumber, TimePO buytime,
		...
		CarVO vo=new CarVO(vehicle, id+"", engine, carNum, basenumber,  buytime,
				usetime);
		voList.add(vo);
	...
	public boolean Upate(ArrayList<CarVO> voList) {
		...
		for(int i = 0; i<voList.size();i++){	
			...
			CarPO po=new CarPO(Vehicle.CAR,vo.getName(),vo.getEngine(),vo.getCarname(),vo.getBasenumber(),vo.getBuytime(),vo.getUsetime());
