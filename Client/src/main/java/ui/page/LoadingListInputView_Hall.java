			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result == true) {
					...
					LogBLService.insert(TimePO.getNowTimePO(),
							po.getPermission().toString() + po.getUsername() + "提交了新的装车信息");
				} else {
		...
		if (loadMonitorField.getText().equals("") || loadPerformerField.getText().equals("")) {
			...
			try {
				...
				if (order != null) {
					...
					LoadingVO lv = bl.addLoading(ser.getTimePO(), Long.parseLong(centerNumField.getText()),
					...
					LogBLService.insert(TimePO.getNowTimePO(),
							po.getPermission().toString() + po.getUsername() + "添加了订单：" + id + "的装车信息");
