...
public class XTimeChooser extends JPanel {
	private TimePO time;
	private Calendar calendar;
	private Calendar now = Calendar.getInstance();
	private JPanel calendarPanel;
	private java.awt.Font font = XContorlUtil.FONT_14_BOLD;
	...
	private static int defaultStartDAY = 0;// 0 is from Sun, 1 is from Mon, 2 is
											// from Tue
	private static Color hoverColor = XContorlUtil.SELECTED_TEXT_COLOR; // hover
																		// color

	private XTimeChooser(java.util.Date date, String format, int startDAY) {
	...
	public static XTimeChooser getInstance(java.util.Date date, String format) {
		return new XTimeChooser(date, format, defaultStartDAY);
	}

	public static XTimeChooser getInstance(java.util.Date date) {
		return getInstance(date, DEFAULTFORMAT);
	}

	public static XTimeChooser getInstance(String format) {
		return getInstance(new java.util.Date(), format);
	}

	public static XTimeChooser getInstance() {
		...
		showComponent.addMouseListener(new MouseAdapter() {
			...
			public void mouseExited(MouseEvent me) {
				if (showComponent.isEnabled()) {
					...
					showComponent.setForeground(XContorlUtil.DEFAULT_PAGE_TEXT_COLOR);
				}
			}
	...
	private class TitlePanel extends JPanel {
		...
		public TitlePanel() {
			...
			this.setBackground(new java.awt.Color(250, 250, 251));
			initTitlePanel();
		}
		...
		class MyMouseAdapter extends java.awt.event.MouseAdapter {
			...
			public void mouseExited(java.awt.event.MouseEvent me) {
				...
				label.setForeground(XContorlUtil.DEFAULT_PAGE_TEXT_COLOR);
			}
	...
	private class FooterPanel extends JPanel {
		...
		private void initFooterPanel() {
			dateLabel = new JLabel("今天是 : " + sdf.format(new java.util.Date()));
			final Color defaultColor = dateLabel.getForeground();
			dateLabel.addMouseListener(new MouseListener() {

				@Override
				public void mouseReleased(MouseEvent e) {
					dateLabel.setForeground(defaultColor);
				}
	...
	private class DayLabel extends JLabel implements java.util.Comparator<DayLabel>, java.awt.event.MouseListener,
		...
		public DayLabel(Calendar cal) {
			...
			if (month == calendar.get(Calendar.MONTH))
				this.setForeground(XContorlUtil.DEFAULT_PAGE_TEXT_COLOR);
			else
		...
		protected void paintComponent(Graphics g) {
			...
			if (day == calendar.get(Calendar.DAY_OF_MONTH) && month == calendar.get(Calendar.MONTH)) {
				setForeground(XContorlUtil.MENUITEM_BACKGROUND);
				g.setColor(XContorlUtil.SELECTED_TEXT_COLOR);
			...
			if (year == now.get(Calendar.YEAR) && month == now.get(Calendar.MONTH)
				...
				gd.setColor(XContorlUtil.DEFAULT_PAGE_TEXT_COLOR);
	...
	private class LabelManager {
		...
		public LabelManager() {
			list = new ArrayList<XTimeChooser.DayLabel>();
		}
	...
	private void setTimePO(Calendar calendar) {
		...
		time = new TimePO(date.getYear() + 1900, date.getMonth() + 1, date.getDate(), date.getHours(),
