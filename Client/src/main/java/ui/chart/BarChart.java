...
public class BarChart extends XChartPanellet {
	...
	public BarChart() {
		JFreeChart jfreechart = ChartFactory.createBarChart("柱子可点击", "Years", "Monery", getDataset(),
		...
		/*
		 * 显示进度 //loadlLabel.setIcon(XContorlUtil.getImageIcon(
		 * "ui/images/chartpanel/loading.gif"));
		 * loadlLabel.setIcon(XContorlUtil.getImageIcon("ui/images/loading.gif")
		 * ); add(loadlLabel); //new LoadDataWorker().execute();
		 */
	}
	...
	/*
	 * Swingworker不同时启动多个线程，性能不够 class LoadDataWorker extends
	 * SwingWorker<CategoryDataset, Void> {
	 * 
	 * @Override protected CategoryDataset doInBackground() throws Exception {
	 * return getCategoryDataset(); }
	 * 
	 * @Override protected void done() { try { dataset = get(); JFreeChart
	 * jfreechart = ChartFactory.createBarChart("Housing Consume", "Years",
	 * "Monery", dataset, PlotOrientation.VERTICAL, true, true, false);
	 * setChart(jfreechart); } catch (InterruptedException e) {
