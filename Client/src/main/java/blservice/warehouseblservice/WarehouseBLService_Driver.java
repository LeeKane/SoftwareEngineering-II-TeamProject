...
public class WarehouseBLService_Driver {
	public void drive(WarehouseBLService warehouseBLService) {
		ResultMessage result = warehouseBLService
				.setPlaneShipping(new ListPO(ListType.TRANS, 1, ListState.SUBMITTED, new TimePO(1, 1, 1, 1, 1, 1)));
		if (result == ResultMessage.SUCCESS)
			System.out.println("SetPlaneShipping Succeeded!");

		result = warehouseBLService
				.setCarShipping(new ListPO(ListType.TRANS, 2, ListState.SUBMITTED, new TimePO(1, 1, 1, 1, 1, 1)));
		if (result == ResultMessage.SUCCESS)
			System.out.println("SetCarShipping Succeeded!");

		result = warehouseBLService
				.setTrainShipping(new ListPO(ListType.TRANS, 3, ListState.SUBMITTED, new TimePO(1, 1, 1, 1, 1, 1)));
		if (result == ResultMessage.SUCCESS)
