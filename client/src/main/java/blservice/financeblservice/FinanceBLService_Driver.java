...
public class FinanceBLService_Driver {
	public void drive(FinanceBLService financeBLService){
		ResultMessage balanceman = financeBLService.BalanceMan(new ReceiptPO(new StaffPO(1,"haha",  OrgType.CENTER, Permission.COURIER), new InstitutePO(City.BEIJING, OrgType.CENTER, 1), 1, new TimePO(1,1, 1, 1, 1, 1)));
		if(balanceman==ResultMessage.SUCCESS)
			System.out.println("BalanceManagement Succeeded!");
		
		ResultMessage costman= financeBLService.CostMan(new BaccountPO("haha", "asd", "piid"), new CostchartPO(ListType.MONEYIN, new TimePO(1,1, 1, 1, 1, 1), 1, "haha", "shar", 1, new TimePO(1,1, 1, 1, 1, 1), 1, 1, 1, new TimePO(1,1, 1, 1, 1, 1), 1, RewardType.COUNT));
		if(costman==ResultMessage.SUCCESS)
			System.out.println("CostManagement Succeeded!");
		
		ResultMessage statisticalchart = financeBLService.StatisticalChart(new StatisticalchartPO(false, ListType.MONEYIN, ResultMessage.SUCCESS, new TimePO(1,1, 1, 1, 1, 1)));
